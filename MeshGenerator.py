...
def createMesh(radius, pitch, turns, tube_r, n_points, n_circle_pts, lc, mesh_name, visualize = False):
    gmsh.initialize()
    ...
    # Get all surfaces (dimension 2) after fusion
    surfaces_after_fuse = gmsh.model.getEntities(2)
    surface_tags = [s[1] for s in surfaces_after_fuse]

    # Make a surface loop and volume
